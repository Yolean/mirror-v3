...
| **5b** | Restart-correctness e2e | `e2e/tests/restart_correctness.rs` proves abort + restart resumes at `max(to)+1` with no gaps or duplicates against real Kafka |
| **2b** (deferred) | Toxiproxy fault injection in the Docker stack | The `Provisioner`/`ProvisionedStack::inject_fault` seam is in place; a Toxiproxy-based impl is the next infra investment. Restart-correctness is now covered without it (see 5b) so this is lower priority than originally framed. |
